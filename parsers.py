import requests
from settings import AWPROXY_PROXY_LIST_URL, RSOCKS_PROXY_LIST_URL, NOSOK_PROXY_LIST_URL, BEST_PROXY_LIST_URL

def get_provider_proxies(url, name):
    html = requests.get(url).text
    return [{"name": name, "address": proxy} for proxy in html.strip().split('\n')]
